...
-- ═══════════════════════════════════════════════════════════════════════════
-- BUTTON CACHE: scan ElvUI + Blizzard action bars
-- ═══════════════════════════════════════════════════════════════════════════
...
local function RebuildBindMap()
  ...
  for _, btn in ipairs(buttonCache) do
    if not btn:IsForbidden() and btn.action then
      ...
      if spellID or itemID then
        local slotNum = slot % 12
        if slotNum == 0 then slotNum = 12 end
        local bindAction = (btn.GetBindingAction and btn:GetBindingAction())
          or (btn.buttonType and btn.buttonType .. slotNum)
          or ("ACTIONBUTTON" .. slotNum)

        local binds = { GetBindingKey(bindAction) }
        for _, bind in ipairs(binds) do
          if not bindMap[bind] then
            bindMap[bind] = { spellID = spellID, itemID = itemID, buttons = {} }
          end
          tinsert(bindMap[bind].buttons, btn)
        end
      end
    end
  end
